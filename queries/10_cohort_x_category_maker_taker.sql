-- Cohort × category × side breakdown (30d).
--
-- Outputs three columns of dollar volume per (cohort, category):
--   maker_vol_musd  — dollars where the cohort wallet was the MAKER
--   taker_vol_musd  — dollars where the cohort wallet was the TAKER
--   touched_vol_musd — maker + taker (the "combined participation" stat
--                      used in the headline cohort x category matrix)
--
-- Use case: distinguishing who PROVIDES liquidity vs who CONSUMES
-- liquidity within each category. The touched/aggregate view in
-- 04_cohort_x_category_30d.sql conflates these.
--
-- Cost: ~30 credits, similar to query 09.
...
SELECT
  ...
  ws.category,
  ROUND(SUM(CASE WHEN ws.side='maker' THEN ws.notional ELSE 0 END) / 1e6, 2) AS maker_vol_musd,
  ROUND(SUM(CASE WHEN ws.side='taker' THEN ws.notional ELSE 0 END) / 1e6, 2) AS taker_vol_musd,
  ROUND(SUM(ws.notional) / 1e6, 2) AS touched_vol_musd
FROM wallet_sides ws
JOIN cohorts c ON c.wallet = ws.wallet
GROUP BY 1, 2
